...
class DbManager extends BaseManager
{
    ...
    {       
        if($this->checkIsAssign($item->name)){
            return false;exit;
        }
    ...
    /**
     * 检测权限是否被分配
     * @param type $itemName 权限名称
     * @return boolean
     */
    protected function checkIsAssign($itemName) {
        $sql="select parent from {$this->itemChildTable} where child='{$itemName}'";
        $res=Yii::$app->db->createCommand($sql)
           ->queryAll();        
        if(empty($res)){
           $sql2="select * from {$this->assignmentTable} where item_name='{$itemName}'";
...
//               return $res2;exit;
            ...
            foreach ($res as $key => $value) {
                return $this->checkIsAssign($value['parent']);
            }
        }
    }
    /**
