...
class JsSdk extends Wxbase
{
    ...
    {
        $ticketPath = \Yii::$app->params['store_path'] . "jsapi_ticket.json";
        $is_file = file_exists($ticketPath);
        if ($is_file) {
            $data = json_decode(file_get_contents($ticketPath));
        }else{
            $data = new \stdClass();
        }
